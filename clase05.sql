...
-- operadores AND y OR
select * from clientes where id < 3 or id > 5;
select * from clientes where id >=3 and id <=5;

-- valores NULL
-- insertamos registros para ver las consultas con valores NULL
insert into clientes (nombre, apellido, dni, direccion) values ('Mara', 'Gamarra', '12345678', '');
insert into clientes (nombre, apellido, dni, direccion) values ('Lucas', 'Pato', '12345668', null);

-- NULL significa ausencia de dato o dato desconocido. No es lo mismo que 0 (cero), ni que una cadena vacìa (''), ni que un espacio.
-- tener en cuenta en las operaciones que un null va a dar como resultado siempre null.
select * from clientes where direccion = '';
select * from clientes where direccion is null; -- trae todos los registros cuya dirección sea null
select * from clientes where direccion is not null; -- trae a todos los registros cuya dirección no sea null
...
select * from facturas where monto between 30000 and 40000;
select * from facturas where monto not between 30000 and 40000;
-- la sentencia between incluye los extremos
-- la sentencia not between no incluye los valores extremos
insert into facturas values ('A',1000,curdate(),30000);
insert into facturas values ('A',1,curdate(),40000);

-- parámetros IN y NOT IN
select * from clientes where id in (2,4,6); -- id=2 OR id=4 OR id=6
select * from clientes where id not in (2, 4, 6); -- id<>2 AND id<>4 AND id<>6

-- parámetros LIKE y operadores % y _
...
select * from clientes order by nombre asc; -- ordena alfabéticamente por nombre en forma ascendente
select * from clientes order by nombre; -- hace lo mismo que el anterior sin necesidad de agregar el asc
-- no es obligatorio usar el ASC, pero si el desc
select * from clientes order by nombre desc; -- los ordena en forma alfabética descendente
select * from clientes order by apellido desc, nombre; -- ordena por apellido descendente y luego por nombre ascendente.

-- cláusula LIMIT
select * from clientes limit 10; -- muestra los primeros 10 registros
select * from clientes limit 3,2; -- trae desde después del 3er registro, dos números

-- función de redondeo
select PI(); -- devuelve el valor de PI -> 3.141593
select round(PI()); -- devuelve la parte entera
select round(PI(),2);  -- redondea a 2 decimales
