...
select * from clientes where direccion is null; -- cuando buscamos por valores nulos o no nulos ponemos IS NULL o IS NOT NULL
...
-- Si no usamos GROUP BY, devuelven un único resultado general
...
-- ---------------------------------------------------------------------
-- SUBQUERIES
SELECT MAX(monto) from facturas; -- trae el máximo monto facturado.
SELECT * FROM facturas WHERE monto = ( SELECT MAX(monto) from facturas);
-- trae todos los registros cuyo monto sea igual al máximo monto facturado
SELECT * FROM facturas WHERE fecha = (select MAX(fecha) from facturas);
-- trae todos los registros cuya fecha sea la más reciente
